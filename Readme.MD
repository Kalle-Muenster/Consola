...
Any other thread will be able writing StdStream.Out again as soon either *stream.End()* is called, or at least with *stream* leaving the actual scope. (it then would call *.End()* automatically as soon it's catched by the collector, on disposal) 
