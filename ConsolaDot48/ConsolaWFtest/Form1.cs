...
namespace StdStreamsWFtest
{
    public partial class Form1 : Form
    {
        ...
        private Consola.StdStreams std = null;

        private void OutputTask(object ident) {
            string taskname = (ident as object[])[0] as string;
            int cycletime = (int)(ident as object[])[1];
            do { Consola.StdStream.Out.WriteLine( "This is task '{0}' writing to stdout: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", taskname );
        ...
        private void ErrorTask(object ident) {
            string taskname = (ident as object[])[0] as string;
            int cycletime = (int)(ident as object[])[1];
            do { Consola.StdStream.Err.WriteLine( "This is task '{0}' writing to stderr: 111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111", taskname );
                Thread.Sleep( cycletime );
            } while ( (running & 1u) == 1u );
        }
        ...
        private void OutputStream(object ident) {
            string taskname = (ident as object[])[0] as string;
            int cycletime = (int)(ident as object[])[1];
            do { Consola.StdStream.Out.Stream.Put( "This is task '" ).Put( taskname ).Put( "' writing to stdout: " ).Put("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\n").End();
                Thread.Sleep( cycletime );
            } while ( (running & 2u) == 2u );
        }
        private void ErrorStream(object ident) {
            string taskname = (ident as object[])[0] as string;
            int cycletime = (int)(ident as object[])[1];
            do { Consola.StdStream.Err.Stream.Put( "This is task '" ).Put( taskname ).Put( "' writing to stderr " ).Put("222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222\n").End();
                Thread.Sleep( cycletime );
            } while ( (running & 2u) == 2u );
        }

        private void InputTask(object taskObj) {
            do { if ( getcha.Length == 0 ) {
        ...
        public Form1()
        {
            ...
            string vers = Consola.Utility.VersionString;
            lbl_version.Text = string.Format("{0}, {1}",vers,ver);
        }

        private void button1_Click(object sender, EventArgs e)
        {
            Consola.StdStream.Init( Consola.CreationFlags.TryConsole );
            Consola.StdStream.Out.WriteLine("This is a test!");
        }

        private void button2_Click(object sender, EventArgs e)
        {
            if (std == null)
                std = new Consola.StdStreams( Consola.CreationFlags.UseConsole );
        ...
        {
            Consola.StdStream.Init( Consola.CreationFlags.TryConsole );
            string inp = Consola.StdStream.Inp.ReadLine();
            textBox1.Text += inp;
        }
        ...
        {
            labeltext = "";
            textBox1.Text = string.Format("processid: {0} - ", Consola.Utility.CommandLine( textBox1.Text, getResult ) );
            Paint += Form1_Paint;
        }
